...
Block {
  ...
  property var listTitle
    function d1(list,data)
    {
        if (list === undefined)
            return;
        var block = Qt.createQmlObject('import HTML 1.0;Block{}', c1)

        for (var i = 0; i < list.length; i++)
          {
            var key = list[i]
            var value = data[key]
            var a = Qt.createQmlObject('import HTML 1.0;A{className: "list-group-item list-group-item-action active"}', block)
            a.setHref("#"+key)
            a.setText(value)
            block.appendBlock(a)
          }
    ...
    {
        var block = Qt.createQmlObject('import HTML 1.0;Block{}', c2)
